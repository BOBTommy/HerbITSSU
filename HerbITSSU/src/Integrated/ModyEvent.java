...
public class ModyEvent {
	...
	private String[] categoryS = { "바디/헤어", "컵", "티슈/여성", "팩/베겟속", "베이비",
			"화장품", "아로마/오일/램프", "방향제/향수", "향초/홀더", "비누/쿠키", "직원용", "커피",
			"아이스커피", "음료", "과일쥬스", "빙수/와플/캔디", "티포트", "테이크아웃", "워머/핸드밀",
			"원두/허브차" };
	private String n;
 ...
 {
					if (n.equals(menupane.returnName())) {
						try {
							...
							os.db.exec("update herb_menu set menu_name=" + "'"
									...
									+ "where menu_id=" + rs.getInt("menu_id"));
		...
		delet.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {	
...

				{
					if (n.equals(menupane.returnName())) {
						try {
							os.db.exec("delete from herb_menu where menu_id="
									+ rs.getInt("menu_id"));
						} catch (SQLException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
	...
	private void search() {
		...
		if (n == null) {
			...
			name.setText(n);
			rs = os.db.exec("select * from herb_menu where menu_name = '" + n
					+ "'");
			try {
				rs.next();
				if (rs.wasNull()) {
